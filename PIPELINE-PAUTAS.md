...
## Flujo semanal en el Mac (desde semana 37 — 2026-09-13)

Scripts generalizados (todos en `_src/`, rutas relativas al repo; ya no hay rutas Windows):

1. Copiar el Excel a `descargas/PROGRAMA_CON_LUB_Y26W<nn>.xlsx`.
2. `python3 parse_plan.py <nn>` → `plan_w<nn>.json` (mapea tag+desc contra todas las
   semanas ya verificadas) e imprime las OT pendientes de Máximo.
3. Máximo (pestaña `teck.maximo.com/maximo/` con sesión): `mxapiwodetail` con
   `oslc.where=wonum in [...]` y `oslc.select=wonum,worktype,status,doclinks`; el campo
   `doclinks` es un objeto `{href}` — los nombres de archivo salen con
   `fetch(doclinks.href+'?lean=1&oslc.select=*')` por OT (`document`/`urlname`).
   Guardar como `maximo_w<nn>.txt` con líneas `OT | TIPO | ARCHIVOS`.
4. `python3 apply_maximo.py <nn>` → aplica al plan y lista los PLN nuevos.
5. PLN nuevos: primero la biblioteca local del traspaso (`~/Documents/Aplicacion lubricación
   traspaso/.../pautas-xlsx/`); los que falten, SharePoint search + `download.aspx`
   (ver puntos 2-3 de arriba). Dejarlos en `descargas/pautas-xlsx/`.
6. `python3 extract_pauta.py <xlsx> <PLN>` por archivo → `python3 build_pauta_html.py <PLN…>`
   → `python3 qa_cellmaps.py` y `python3 qa_htmls.py`.
7. `python3 build.py && python3 -W ignore publicar_datos.py && git add -A && git commit && git push`.
   Desde sep-2026 los datos y las réplicas HTML NO van en el sitio: `publicar_datos.py`
   los sube a Firestore (colección `datos`) y la app los carga tras el login.
