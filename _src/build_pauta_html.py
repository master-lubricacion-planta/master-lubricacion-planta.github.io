...
SCRATCH = os.path.dirname(os.path.abspath(__file__))   # _src (pautas.json vive aqui)
PWA = os.path.dirname(SCRATCH)                          # raiz del repo
DL = os.path.join(PWA, 'descargas', 'pautas-xlsx')      # xlsx nuevos bajados de SharePoint/Maximo
...
def main():
    ...
    # biblioteca historica de xlsx (146 pautas del traspaso desde Windows)
    ALT_DIR = os.path.expanduser('~/Documents/Aplicacion lubricación traspaso/Traspaso-Lubricacion-Mac/pautas-xlsx')
    # uso: python3 build_pauta_html.py [PLN-id ...]  -> solo esas; sin argumentos, todas
    solo = set(sys.argv[1:])
    for pln_id in list(by_id.keys()):
        if solo and pln_id not in solo:
            continue
